...
class PointCloudPlayer:
    ...
    def get_datas(self):
        ...
        for name in os.listdir(self.velodyne_path):
            ...
            self.labels.append(
                np.fromfile(label_file, dtype=np.uint32).reshape((-1)) & 0xFFFF
            )
        ...
        with tqdm(total=len(self.pc_datas)) as pbar:
            ...
            for pc_data, label in zip(self.pc_datas, self.labels):
                ...
                pcd.colors = o3d.utility.Vector3dVector(
                    self.draw_color(point_xyz, label)
                )
...
if __name__ == "__main__":
    use_pred = True
    dataset_root_path = (
        "/data1/MotionSeg3d_Dataset/data_odometry_velodyne/dataset/sequences"
    )
    ...
    pcp = PointCloudPlayer(
        velodyne_path=os.path.join(dataset_root_path, seq, "velodyne"),
