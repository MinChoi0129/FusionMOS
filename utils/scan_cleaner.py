...
if __name__ == "__main__":
    ...
    data_yaml_filename = "config/labels/semantic-kitti-mos.raw.yaml"
    if len(sys.argv) > 1:
    ...
    if yaml.__version__ >= "5.1":
        ...
        data_yaml = yaml.load(open(data_yaml_filename))
    ...
    if split == "train":
        for seq in data_yaml["split"]["train"]:
            ...
            if not os.path.exists(
                os.path.join(clean_scan_root, "sequences", seq, "clean_scans")
            ):
                os.makedirs(
                    os.path.join(clean_scan_root, "sequences", seq, "clean_scans")
                )
