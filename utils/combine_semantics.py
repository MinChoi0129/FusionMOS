...
if __name__ == "__main__":
    ...
    if split == "train":
        ...
        for seq in data_yaml["split"]["train"]:
            ...
            if not os.path.exists(
                os.path.join(combined_results_root, "sequences", seq, "predictions")
            ):
                os.makedirs(
                    os.path.join(combined_results_root, "sequences", seq, "predictions")
                )
    ...
    if split == "valid":
        for seq in data_yaml["split"]["valid"]:
            ...
            print("valid", seq)
            if not os.path.exists(
                os.path.join(combined_results_root, "sequences", seq, "clean_scans")
            ):
                os.makedirs(
                    os.path.join(combined_results_root, "sequences", seq, "clean_scans")
                )
    ...
    for seq in seqs:
        ...
        semantic_pred_seq_path = os.path.join(
            semantic_pred_root, "sequences", seq, "predictions"
        )
        ...
        for frame_idx in tqdm(range(len(mos_pred_files))):
            mos_pred, _ = load_labels(
                mos_pred_files[frame_idx]
            )  # mos_pred should be 9/251 for static/dynamic
            semantic_pred, _ = load_labels(
                semantic_pred_files[frame_idx]
            )  # mos_pred should be full classes
