...
if __name__ == "__main__":
    ...
    class_remap = DATA["moving_learning_map"]
    class_inv_remap = DATA["moving_learning_map_inv"]
    ...
    for sequence in test_sequences:
        ...
        if FLAGS.radius != -1:
            ...
            seq_lidar_names = [
                os.path.join(dp, f)
                for dp, dn, fn in os.walk(os.path.expanduser(lidar_paths))
                for f in fn
                if ".bin" in f
            ]
    ...
    for f_id in tqdm(
        range(len(label_names[:])), desc="Evaluating sequences:", ncols=80
    ):
        ...
        if FLAGS.radius != -1:
            pc_xyz = np.fromfile(lidar_names[f_id], dtype=np.float32).reshape((-1, 4))[
                :, :3
            ]
