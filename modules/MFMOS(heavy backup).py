...
class MFMOS(nn.Module):
    def __init__(
        ...
        self.bev_conv1x1_layer1_channel_wise = nn.Conv2d(
            128, 128, 1, bias=True)
        self.bev_conv1x1_layer1_spatial = nn.Conv2d(128, 1, 1, bias=True)
        self.bev_conv1x1_layer2_channel_wise = nn.Conv2d(
            256, 256, 1, bias=True)
        self.bev_conv1x1_layer2_spatial = nn.Conv2d(256, 1, 1, bias=True)
        self.bev_conv1x1_layer3_channel_wise = nn.Conv2d(
            256, 256, 1, bias=True)
    ...
    def forward(self, x_range, x_bev):
        ...
        BirdEyeView_down0c, BirdEyeView_down0b = self.bev_RI_resBlock1(
            bev_downCntx)
...
if __name__ == "__main__":
    ...
    x_range = torch.rand(
        ...
        params["train"]["n_input_scans"], 64, 2048
    )
    x_bev = torch.rand(
        params["train"]["batch_size"], 5 +
        params["train"]["n_input_scans"], 384, 384
    )
    if torch.cuda.is_available():
