    ...
    def train_epoch(
        ...
        for i, (
            ...
            moving_loss_m = criterion(
                torch.log(output.clamp(min=1e-8)).double(), proj_labels
            ).float() + self.ls(output, proj_labels.long())
            movable_loss_m = movable_criterion(
                torch.log(movable_output.clamp(min=1e-8)).double(), movable_proj_labels
            ).float() + self.movable_ls(movable_output, movable_proj_labels.long())
            ...
            if show_scans:
                show_scans_in_training(proj_mask, in_vol, argmax, proj_labels, color_fn)
                show_scans_in_training(
                    proj_mask,
                    in_vol,
                    movable_argmax,
        ...
        with torch.no_grad():
            ...
            for i, (
                ...
                if self.gpu:
                    ...
                    movable_proj_labels = movable_proj_labels.cuda(
                        non_blocking=True
                    ).long()
                ...
                movable_wce = movable_criterion(
                    movable_log_out.double(), movable_proj_labels
                ).float()
